...
def show_stimuli_with_collected_language(
    stimulus, collected_data_df=None, show_lang=True, print_dataset=False, validation=False
):
    ...
    if show_lang and collected_data_df is not None:
        ...
        for i in range(len(stim_data)):
            if print_dataset:
                print(
                    bold_string("Correct guess= " + str(guesses.iloc[i])),
                    dataset.iloc[i],
                    utterances.iloc[i],
                )
            else:
