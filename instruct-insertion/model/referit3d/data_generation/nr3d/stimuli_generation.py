...
class SameInstanceStimulus(object):
    ...
    def __init__(
        self, scan_id, target_id, distractors_ids, instance_types, target_bbox, distractor_bboxes
    ):
    ...
    def __str__(self):
        res = "-".join(
            [str(x) for x in [self.scan_id, self.instance_types, len(self), self.target_id]]
        )
        if len(self) > 1:
...
class SameInstanceSyntheticStimulus(SameInstanceStimulus):
    def __init__(
        ...
        super().__init__(
            scan_id, target_id, distractors_ids, target_instance, target_bbox, distractor_bboxes
        )
        self.type = ref_type
...
def generate_stimuli(scans, target_instance_type, must_be_multiple=True, too_hard=None):
    ...
    hits = dict()
    for scan in scans:
        targets = Reference.valid_targets(
            scan, must_be_multiple=must_be_multiple, valid_instances=target_instance_type
        )
        ...
        for i in range(n_objects):
            ...
            hit = SameInstanceStimulus(
                scan.scan_id,
                target_ids[i],
                distractor_ids,
                target_instance_type[0],
                bboxs[i],
                bboxs[distractor_idx],
            )
            #             print(hit)
...
def is_valid_hit(
    hit, max_coverage_threshold=1, max_iou_threshold=1, manual_black_list=None, all_pairs=False
):
