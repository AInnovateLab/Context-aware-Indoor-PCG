...
import numpy as np
import pandas as pd
from referit3d.data_generation.sr3d import (
    AllocentricGenerator,
    BetweenGenerator,
    HPGenerator,
    SupportAndVPGenerator,
)
from referit3d.in_out.scan_2cad import (
    load_has_front_meta_data,
    load_scan2cad_meta_data,
...
from referit3d.utils import read_lines, str2bool, unpickle_data
...
SCAN2CAD_META_FILE = (
    "../../data/scan2cad/object_oriented_bboxes/object_oriented_bboxes_aligned_scans.json"
)
SCAN2CAD_PATCHED_META_FILE = (
    "../../data/scan2cad/object_oriented_bboxes/patched_object_oriented_bboxes_aligned_scans.json"
)
...
def parse_args():
    ...
    parser.add_argument(
        "-preprocessed_scannet_file",
        type=str,
        help=".pkl (output) of prepare_scannet_data.py",
        required=True,
    )
    parser.add_argument(
        "-valid_targets_file",
    ...
    parser.add_argument(
        "-name", type=str, help="Name of the gerneated sr3d csv file.", required=True
    )
    parser.add_argument("--verbose", type=str2bool, default=True, help="verbose")
    parser.add_argument(
        "--stimulus_is_too_hard",
        type=int,
        default=6,
...
def sample_and_save_synthetic_utterances(args, references, all_scans, synth_to_human_lang_dict):
    ...
    for ref in references:
        ...
        dummy_target_label, distractor_ids, stimulus_id = scan_and_target_id_to_context_info(
            scan_id, target_id, all_scans_in_dict
        )
        assert target_instance_label == dummy_target_label

        anchors_instance_labels = [i.instance_label for i in ref.anchors()]

        utterances = ref.to_human_language(
            ref, synth_to_human_lang_dict, n_utterances=args.max_samples_per_context
        )
        ...
        example = [
            {
                ...
                "anchor_ids": [i.object_id for i in ref.anchors()],
            }
...
def generate_synthetic_references():
    ...
    params = [
        ...
        valid_targets,
        valid_anchors,
        args.targets_must_be_multiple,
        args.stimulus_is_too_hard,
    ]
    ...
    return horizontal_proximity_refs + allocentric_refs + vertical_proximity_refs + between_refs
