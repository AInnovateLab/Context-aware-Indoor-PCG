...
class Reference(object):
    ...
    def __init__(
        self, target: ThreeDObject, anchor: ThreeDObject, reference_type: str, second_anchor=None
    ):
        """
    ...
    def distractors(self) -> list:
        ...
        for o in scan.three_d_objects:
            if (
                o.instance_label == self.target.instance_label
                and o.object_id != self.target.object_id
            ):
                res.append(o)
    ...
    def __str__(self, verbose=True):
        res = "{}: {}({}), {}({})".format(
            self.type,
            self.target.instance_label,
            self.target.object_id,
            self.anchor.instance_label,
    ...
    @staticmethod
    def valid_targets(
        scan, must_be_multiple=True, valid_instances=None, group_by_label=False, too_hard=None
    ):
        """
        Returns a list of three_d object if not group_by_label, otherwise returns dict.
    ...
    def force_uniqueness(all_refs) -> list:
        ...
        for ref in all_refs:
            if ref.second_anchor is not None:
                key = [
                    ref.target.instance_label,
                    ref.anchor.instance_label,
                    ref.second_anchor.instance_label,
                ]
            else:
            ...
            if ref.type in [
                ...
                "above",
                "below",
            ]:  # To reduce the effect of the bad annotations of the bounding boxes
                key.append("vertical")
            else:
                key.append(ref.type)
    ...
    def sample_from_template_set(reference, template_set, template_dict):
        ...
        if reference.get_reference_type(coarse=True) == "between":
            ...
            relations = template_dict["relations"][
                reference.get_reference_type(coarse=False)
            ].split("/")
    ...
    def to_human_language(reference, template_dict: dict, n_utterances: int) -> list:
        ...
        for t_set in template_sets:
            ...
            if reference.satisfies_template_conditions(conditions, t_set):
                utterances.extend(
                    Reference.sample_from_template_set(reference, t_set, template_dict)
                )
    ...
    def deserialize(all_scans, serialized_ref):
        ...
        return Reference(
            target=target, anchor=anchor, reference_type=reference_type, second_anchor=second_anchor
        )
