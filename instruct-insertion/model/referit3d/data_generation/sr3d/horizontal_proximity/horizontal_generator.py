...
class HorizontalProximityGenerator(ReferenceGenerator):
    ...
    def generate_for_single_scan(self, scan):
        ...
        instance_to_targets = defaultdict(
            list
        )  # Group same-instance type, since you care only about distances
        ...
        for anchor in all_anchors:
            for instance_type in instance_to_targets.keys():
                ...
                if all_dists[0] + epsilon_gap < all_dists[1]:  # "closest" ref-type
                    ref = Reference(
                        current_target_group[s_idx[0]], anchor, reference_type="closest"
                    )
                    all_refs.append(ref)
                if all_dists[-1] - epsilon_gap > all_dists[-2]:  # "farthest" ref-type
                    ref = Reference(
                        current_target_group[s_idx[-1]], anchor, reference_type="farthest"
                    )
                    all_refs.append(ref)
