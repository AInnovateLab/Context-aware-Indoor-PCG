...
def evaluate(projectdir, filename_cad_appearance, filename_annotations):
    ...
    benchmark_per_scan = collections.defaultdict(
        lambda: collections.defaultdict(lambda: 0)
    )  # <-- benchmark_per_scan
    benchmark_per_class = collections.defaultdict(
        lambda: collections.defaultdict(lambda: 0)
    )  # <-- benchmark_per_class
    ...
    for file0 in glob.glob(projectdir + "/*.csv"):
        ...
        for alignment in alignments:  # <- multiple alignments of same object in scene
            ...
            if cadkey in appearances_cad[id_scan]:
                n_appearances_allowed = appearances_cad[id_scan][
                    cadkey
                ]  # maximum number of appearances allowed
            else:
            ...
            key = (id_scan, catid_cad)  # <-- key to query the correct groundtruth models_old
            for idx, model_gt in enumerate(groundtruth[key]):
                is_same_class = (
                    model_gt["catid_cad"] == catid_cad
                )  # <-- is always true (because the way the 'groundtruth' was created
                if (
                    is_same_class
                ):  # <-- proceed only if candidate-model and gt-model are in same class
                    Mscan = SE3.compose_mat4(
                        idscan2trs[id_scan]["translation"],
                        idscan2trs[id_scan]["rotation"],
                        idscan2trs[id_scan]["scale"],
                    )
                    Mcad = SE3.compose_mat4(
                        model_gt["trs"]["translation"],
                        model_gt["trs"]["rotation"],
                        model_gt["trs"]["scale"],
                        -np.array(model_gt["center"]),
                    )
                    ...
                    if sym == "__SYM_ROTATE_UP_2":
                        ...
                        tmp = [
                            calc_rotation_diff(
                                q,
                                q_gt
                                * quaternion.from_rotation_vector([0, (i * 2.0 / m) * np.pi, 0]),
                            )
                            for i in range(m)
                        ]
                    ...
                    is_valid_transformation = (
                        error_translation <= threshold_translation
                        and error_rotation <= threshold_rotation
                        and error_scale <= threshold_scale
                    )
    ...
    print("***********")
    benchmark_per_scan = sorted(
        benchmark_per_scan.items(), key=lambda x: x[1]["n_good"], reverse=True
    )
    ...
    for k, v in benchmark_per_scan:
        if "seen" in v:
            ...
            print(
                "id-scan: {:>20s} \t n-cads-positive: {:>4d} \t n-cads-total: {:>4d} \t accuracy: {:>4.4f}".format(
                    k, v["n_good"], v["n_total"], float(v["n_good"]) / v["n_total"]
                )
            )
    instance_mean_accuracy = float(total_accuracy["n_good"]) / total_accuracy["n_total"]
    print(
        "instance-mean-accuracy: {:>4.4f} \t n-cads-positive: {:>4d} \t n-cads-total: {:>4d} \t n-total-scans: {:>4d}".format(
            instance_mean_accuracy,
            total_accuracy["n_good"],
            total_accuracy["n_total"],
            total_accuracy["n_scans"],
        )
    )
    ...
    for k, v in benchmark_per_class.items():
        ...
        print(
            "category-name: {:>20s} \t n-cads-positive: {:>4d} \t n-cads-total: {:>4d} \t accuracy: {:>4.4f}".format(
                k, v["n_good"], v["n_total"], float(v["n_good"]) / v["n_total"]
            )
        )
...
if __name__ == "__main__":
    evaluate(
        opt.projectdir,
        "./" + opt.dataset + "_cad_appearances.json",
        "./" + opt.dataset + "_full_annotations.json",
    )
