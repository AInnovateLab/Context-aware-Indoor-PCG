...
from referit3d.app.init_param import (
    bert_pretrain_path,
    log_dir,
    point_tf_ckpt,
    referit3D_file,
...
from referit3d.utils import create_dir, str2bool
...
def parse_arguments(notebook_options=None):
    ...
    parser.add_argument(
        "--debug_model", type=bool, default=False, help="whether debug in network training"
    )
    parser.add_argument(
        "--batch_pnet", type=bool, default=True, help="batch data for pointnet++ process"
    )
    parser.add_argument(
        "--point_tf_ckpt", type=str, default=point_tf_ckpt, help="ckpt file for point Transformer"
    )
    ...
    parser.add_argument(
        "--add_color", action="store_true", help="Point Transformer + Color Encoder"
    )
    parser.add_argument(
        "--use_pretraining", type=bool, default=True, help="Use pre-trained Point-BERT"
    )
    parser.add_argument(
        "--aug_with_pts", type=bool, default=False, help="use object points to augment cur sample"
    )
    ...
    parser.add_argument(
        "-scannet-file",
        default=scannet_file,
        type=str,
        required=False,
        help="pkl file containing the data of Scannet" " as generated by running XXX",
    )
    parser.add_argument("-referit3D-file", default=referit3D_file, type=str, required=False)

    #
    # I/O file-related arguments
    #
    parser.add_argument(
        "--log-dir", default=log_dir, type=str, help="where to save training-progress, model, etc"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="will be used to name a subdir " "for log-dir if given",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="optional, .pkl file for vocabulary (useful when "
    ...
    if (
        args.resume_path and not args.log_dir and (args.rank == 0)
    ):  # resume and continue training in previous log-dir.
    ...
    print(
        colored("____________________________________________________\n", "red")
    ) if args.rank == 0 else None
    return args
