...
# New eval mode
from referit3d.models.model_dist import evaluate_on_dataset, single_epoch_train
...
from referit3d.utils import create_logger, seed_training_code, set_gpu_to_zero_position
...
def log_train_test_information(epoch):
    ...
    for phase in ["train", "test"]:
        ...
        info = "{} {}: Total-Loss {:.4f}, Listening-Acc {:.4f}".format(
            epoch, phase, meters[phase + "_total_loss"], meters[phase + "_referential_acc"]
        )
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        with tqdm.trange(start_training_epoch, args.max_train_epochs + 1, desc="epochs") as bar:
            ...
            for epoch in bar:
                ...
                if best_test_acc < eval_acc:
                    logger.info(
                        colored("Test accuracy, improved @epoch {}".format(epoch), "green")
                    ) if args.rank == 0 else None
        ...
        if args.dist_train:
            model = nn.parallel.DistributedDataParallel(
                model, device_ids=[args.rank % torch.cuda.device_count()], broadcast_buffers=False
            )
