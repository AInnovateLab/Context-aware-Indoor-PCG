...
import numpy as np
from data.utils import PathLike, invert_dictionary, read_dict_from_json
...
class ScannetDataset(object):
    ...
    def __init__(
        self,
        top_scan_dir: PathLike,
        idx_to_semantic_cls_file: PathLike,
        instance_cls_to_semantic_cls_file: PathLike,
        axis_alignment_info_file: PathLike,
    ):
        self.top_scan_dir = top_scan_dir

        self.idx_to_semantic_cls_dict: Dict[str, str] = read_dict_from_json(
            idx_to_semantic_cls_file
        )

        self.semantic_cls_to_idx_dict = invert_dictionary(self.idx_to_semantic_cls_dict)

        self.instance_cls_to_semantic_cls_dict: Dict[str, str] = read_dict_from_json(
            instance_cls_to_semantic_cls_file
        )
        ...
        self.scans_axis_alignment_matrices: Dict[str, List[float]] = read_dict_from_json(
...
class ScannetScan(object):
    ...
    def __init__(
        self, scan_id: str, scannet_dataset: ScannetDataset, apply_global_alignment: bool = True
    ):
        """
        Args:
            scan_id (str): e.g. 'scene0705_00'
            scannet_dataset (ScannetDataset): reference to the ScannetDataset object. Captures the details about the class-names, top-directories etc.
            apply_global_alignment (bool): whether to apply the global alignment of the scan according to the Scannet meta-data.
        """
        self.dataset = scannet_dataset
        self.scan_id = scan_id
        self.pc, self.semantic_label, self.color = self.load_point_cloud_with_meta_data(
            self.scan_id, apply_global_alignment=apply_global_alignment
        )

        self.three_d_objects: Optional[
            List[ThreeDObject]
        ] = None  # A list with ThreeDObject contained in this Scan
    ...
    def load_point_cloud_with_meta_data(
        self,
        load_semantic_label=True,
        load_color=True,
        apply_global_alignment=True,
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Load the point cloud of the scan and its meta-data (semantic labels, color).

        Args:
            load_semantic_label (bool): whether to load the semantic labels of the points.
            load_color (bool): whether to load the RGB color of the points.
            apply_global_alignment (bool): whether to apply the global alignment of the scan according to the Scannet meta-data.

        Returns:
            pc (np.ndarray): (N, 3) array of the point cloud of the entire scan.
            label (np.ndarray): (N,) array of the semantic labels of the points.
    ...
    def load_point_clouds_of_all_objects(self, exclude_instances=None):
        ...
        aggregation_file = osp.join(
            self.dataset.top_scan_dir, self.scan_id, self.scan_id + scan_aggregation_suffix
        )
        ...
        segment_indices_dict: Dict[int, List[int]] = defaultdict(list)
    ...
    def points_of_instance_types(self, valid_instance_types, exclude_instance_types):
        ...
        for o in self.three_d_objects:
            o_label_valid = (
                True
                if (valid_instance_types is None)
                else (o.instance_label in valid_instance_types)
            )
            o_label_excluded = (
                False
                if (exclude_instance_types is None)
                else (o.instance_label in exclude_instance_types)
            )
    ...
    def sample_indices(
        self, subsample=None, valid_instance_types=None, seed=None, exclude_instance_types=None
    ):
        """
...
def scan_and_target_id_to_context_info(scan_id, target_id, all_scans_in_dict):
    ...
    distractors = [
        x.object_id for x in scene_objects if x.instance_label == instance_label and x != target
    ]
