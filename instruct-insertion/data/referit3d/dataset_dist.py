...
class ReferIt3DDataset(Dataset):
    def __init__(
        ...
        print(
            "\n [INFO]: create Dataset. training={} | use_fps={} | aug_with_pts={}\n".format(
                self.training, self.use_fps, self.aug_with_pts
            )
    ...
    def __getitem__(self, index):
        ...
        samples = np.array(
            [
                sample_scan_object(
                    o, self.points_per_object, self.training, self.use_fps, self.rank
                )
                for o in context
            ]
        )
...
def make_data_loaders(args, referit_data, vocab, class_to_idx, scans, mean_rgb):
    ...
    for split in splits:
        ...
        data_loaders[split] = dataset_to_dataloader(
            args, dataset, split, args.batch_size, n_workers, seed=seed
        )
