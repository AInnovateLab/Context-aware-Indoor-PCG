...
class DGCNN(nn.Module):
    ...
    def get_graph_feature(coor_q, x_q, coor_k, x_k):
        ...
        feature = (
            feature.view(batch_size, k, num_points_q, num_dims).permute(0, 3, 2, 1).contiguous()
        )
...
class PointTransformer(nn.Module):
    ...
    def forward(self, points):
        ...
        neighborhood, center = self.group_divider(
            pts
        )  # pts [32,1024,3] --> neighborhood[B,64,32,3]; center[B,64,3]
        ...
        )  # [1,1,384]-->cls_tokens[B,1,384]
        cls_pos = self.cls_pos.expand(
            group_input_tokens.size(0), -1, -1
        )  # [1,1,384]-->cls_pos[B,1,384]
        # add pos embedding
