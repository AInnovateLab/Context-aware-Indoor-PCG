...
def evaluate_on_dataset(
    model, data_loader, criteria, device, pad_idx, args, randomize=False, tokenizer=None
):
...
@torch.no_grad()
def detailed_predictions_on_dataset(
    model, data_loader, args, device, FOR_VISUALIZATION=True, tokenizer=None
):
    model.eval()
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        predictions = torch.argmax(out["logits"], dim=1)
        res["guessed_correctly_among_true_class"].append(
            (predictions == batch["target_pos"]).cpu().numpy()
        )
