...
parser.add_argument(
    "--output", default="./output/", help="folder to output model checkpoints and predictions"
)
# train params
parser.add_argument("--mask_neg", type=int, required=True, default=1, help="mask negative samples?")
parser.add_argument(
    "--with_match", type=int, required=True, default=1, help="train with negative samples?"
)
parser.add_argument(
    "--with_scale", type=int, required=True, default=1, help="train with scale prediction?"
)
parser.add_argument("--batch_size", type=int, default=16, help="input batch size")
parser.add_argument(
    "--max_iteration", type=int, default=int(1e6), help="number of iteration to train for"
)
parser.add_argument(
    "--n_samples_eval", type=int, default=int(1e6), help="num-max to evaluate of val set"
)
...
gkern3d_dim32 = (
    torch.FloatTensor(torch.from_numpy(kernels.gaussian3d(7, 1.5))).view(1, 1, 7, 7, 7).cuda()
)

model = modelnn.Model3d().cuda()
optimizer = torch.optim.SGD(
    model.parameters(), lr=opt.lr, momentum=opt.momentum, weight_decay=0.0005
)
n_model_params = int(
...
class TrainingWorld:
    ...
    def start(self):
        ...
        last_epoch = (
            self.counter_offset // self.iterations_per_epoch if self.counter_offset > 0 else -1
        )
    ...
    def train(self):
        ...
        self.dataloader_train = torchdata.DataLoader(
            self.dataset_train,
        ...
        for i_batch, samples_batch in enumerate(self.dataloader_train):
            ...
            sys.stdout.write(
                "\rtraining i-iteration: %i / %d         "
                % (self.counter_iteration, opt.max_iteration + self.counter_offset)
            )
            sys.stdout.flush()
    ...
    def save_checkpoint(self):
        torch.save(
            model.state_dict(), os.path.join(projectdir, "cp-%d.pth" % self.counter_iteration)
        )
    ...
    def feed_forward(self, samples):
        ...
        for i in range(opt.batch_size):
            if mask[i]:
                ...
                heatmap2[i] = torch.nn.functional.log_softmax(heatmap0[i].view(-1), dim=0).view(
                    1, 32, 32, 32
                )
                # -> for evalulation
                heatmap3[i] = torch.nn.functional.softmax(heatmap0[i].view(-1), dim=0).view(
                    1, 32, 32, 32
                )
                heatmap3[i] = torch.mul(
                    heatmap1[i] / torch.max(heatmap1[i]), heatmap3[i] / torch.max(heatmap3[i])
                )
                # <-
        ...
        loss0 = losses.weighted_bce_heatmap(
            heatmap1, heatmap_gt, weight=opt.weight, batch_mask=mask
        )
        loss1 = losses.weighted_cross_entropy_heatmap(
            heatmap2, heatmap_gt, weight=opt.weight, batch_mask=mask
        )
    ...
    def evaluate(self):
        ...
        def run(source_list, n_max_samples=-1):
            ...
            self.dataloader = torchdata.DataLoader(
                self.dataset,
                batch_size=opt.batch_size,
                shuffle=False,
                num_workers=opt.n_threads,
