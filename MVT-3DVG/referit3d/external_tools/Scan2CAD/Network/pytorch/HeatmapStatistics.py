...
class Statistics:
    ...
    def update(self, x, gt):
        ...
        for t in self.t0:
            ...
            x_expanded = torch.nn.functional.max_pool3d(
                x_binarized, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size // 2
            )
            gt_expanded = torch.nn.functional.max_pool3d(
                gt_binarized, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size // 2
            )
