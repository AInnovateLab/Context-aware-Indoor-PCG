    ...
    def __init__(self, num_points, transforms=None, train=True, download=True):
        ...
        if not osp.exists(self._cache):
            ...
            if download and not os.path.exists(self.data_dir):
                ...
                subprocess.check_call(shlex.split("curl {} -o {}".format(self.url, zipfile)))

                subprocess.check_call(shlex.split("unzip {} -d {}".format(zipfile, BASE_DIR)))
            ...
            for split in ["train", "test"]:
                if split == "train":
                    shape_ids = [
                        line.rstrip()
                        for line in open(os.path.join(self.data_dir, "modelnet40_train.txt"))
                    ]
                else:
                    shape_ids = [
                        line.rstrip()
                        for line in open(os.path.join(self.data_dir, "modelnet40_test.txt"))
                    ]
                ...
                self.datapath = [
                    (
                        shape_names[i],
                        os.path.join(self.data_dir, shape_names[i], shape_ids[i]) + ".txt",
                    )
                ...
                with lmdb.open(
                    ...
                    for i in tqdm.trange(len(self.datapath)):
                        ...
                        txn.put(
                            str(i).encode(),
                            msgpack_numpy.packb(dict(pc=point_set, lbl=cls), use_bin_type=True),
                        )
