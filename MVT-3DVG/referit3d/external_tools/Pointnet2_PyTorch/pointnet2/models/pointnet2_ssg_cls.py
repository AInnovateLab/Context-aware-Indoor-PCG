...
class BNMomentumScheduler(lr_sched.LambdaLR):
    def __init__(self, model, bn_lambda, last_epoch=-1, setter=set_bn_momentum_default):
        if not isinstance(model, nn.Module):
            raise RuntimeError("Class '{}' is not a PyTorch nn Module".format(type(model)._name_))
...
class PointNet2ClassificationSSG(pl.LightningModule):
    ...
    def _build_model(self):
        ...
        self.SA_modules.append(
            PointnetSAModule(mlp=[256, 256, 512, 1024], use_xyz=self.hparams["model.use_xyz"])
        )
    ...
    def prepare_data(self):
        ...
        self.val_dset = ModelNet40Cls(self.hparams["num_points"], transforms=None, train=False)
