...
class Cuboid(object):
    ...
    def __str__(self):
        return "Cuboid with  [xmin, ymin, zmin, xmax, ymax, zmax] coordinates = %s." % (
            str(self.extrema),
        )
    ...
    def containing_sector(self, sector_center, ignore_z_axis=True):
        ...
        def angle_of_sector(sector_center, side):
            x1, y1, x2, y2 = side
            line_1 = np.array(
                [x1 - sector_center[0], y1 - sector_center[1]]
            )  # First diagonal pair of points of cuboid
...
class OrientedCuboid(object):
    ...
    def _corners(self):
        ...
        axis_aligned_corners = np.hstack(
            [axis_aligned_corners, np.ones((axis_aligned_corners.shape[0], 1))]
        )
        rotation = np.eye(4)
    ...
    def plot(self, axis=None, c="r"):
        ...
        if axis is not None:
            axis.plot(
                [cors[0, 0], cors[1, 0]], [cors[0, 1], cors[1, 1]], zs=[cors[0, 2], cors[1, 2]], c=c
            )
            axis.plot(
                [cors[1, 0], cors[2, 0]], [cors[1, 1], cors[2, 1]], zs=[cors[1, 2], cors[2, 2]], c=c
            )
            axis.plot(
                [cors[2, 0], cors[3, 0]], [cors[2, 1], cors[3, 1]], zs=[cors[2, 2], cors[3, 2]], c=c
            )
            axis.plot(
                [cors[3, 0], cors[0, 0]], [cors[3, 1], cors[0, 1]], zs=[cors[3, 2], cors[0, 2]], c=c
            )
            axis.plot(
                [cors[4, 0], cors[5, 0]], [cors[4, 1], cors[5, 1]], zs=[cors[4, 2], cors[5, 2]], c=c
            )
            axis.plot(
                [cors[5, 0], cors[6, 0]], [cors[5, 1], cors[6, 1]], zs=[cors[5, 2], cors[6, 2]], c=c
            )
            axis.plot(
                [cors[6, 0], cors[7, 0]], [cors[6, 1], cors[7, 1]], zs=[cors[6, 2], cors[7, 2]], c=c
            )
            axis.plot(
                [cors[7, 0], cors[4, 0]], [cors[7, 1], cors[0, 1]], zs=[cors[7, 2], cors[4, 2]], c=c
            )
            axis.plot(
                [cors[0, 0], cors[4, 0]], [cors[0, 1], cors[4, 1]], zs=[cors[0, 2], cors[4, 2]], c=c
            )
            axis.plot(
                [cors[1, 0], cors[5, 0]], [cors[1, 1], cors[5, 1]], zs=[cors[1, 2], cors[5, 2]], c=c
            )
            axis.plot(
                [cors[2, 0], cors[6, 0]], [cors[2, 1], cors[6, 1]], zs=[cors[2, 2], cors[6, 2]], c=c
            )
            axis.plot(
                [cors[3, 0], cors[7, 0]], [cors[3, 1], cors[7, 1]], zs=[cors[3, 2], cors[7, 2]], c=c
            )
            return axis.figure
        else:
            ValueError("NYI")
