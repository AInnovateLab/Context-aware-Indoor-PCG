...
from referit3d.data_generation.nr3d.tokenization import (
    pre_process_text,
...
def parse_arguments():
    ...
    parser.add_argument(
        "-out-file", type=str, required=True, help="the preprocessed data save path"
    )
    ...
    args.__setattr__(
        "word_freq_file", "../../data/language/symspell_frequency_dictionary_en_82_765.txt"
    )
...
def main():
    ...
    if args.type == "nr3d":
        ...
        print(
            "Spell check Loaded:",
            speller.load_dictionary(args.word_freq_file, term_index=0, count_index=1),
        )

        # Preprocess the text
        clean_text, tokens, spelled_tokens, corrected, missed_words = pre_process_text(
            raw_data.utterance,
            sentence_spelling_dictionary,
            token_spelling_dictionary,
        ...
        mentions_target_class_fn = lambda x: mentions_target_class(
            x, all_scans_dict=all_scans_in_dict
        )
        processed_data["mentions_target_class"] = processed_data.apply(
            mentions_target_class_fn, axis=1
        )
        ...
        processed_data = processed_data.assign(
            mentions_target_class=pd.Series([True] * len(processed_data)).values
        )  # by definition
    else:
        raise ValueError
