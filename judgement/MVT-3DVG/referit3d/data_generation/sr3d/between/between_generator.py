...
class BetweenGenerator(ReferenceGenerator):
    ...
    def generate_for_single_scan(self, scan):
        ...
        anchors = Reference.valid_anchors(
            scan=scan,
        ...
        for anc_a, anc_b in itertools.combinations(anchors, 2):
            ...
            for target_label, target_objects in targets.items():
                ...
                for target in target_objects:
                    ...
                    if (
                        t_anc_a < min_target_to_anchor_z_intersection
                        or t_anc_b < min_target_to_anchor_z_intersection
                    ):
                        bad_combinations.append((anc_a.object_id, anc_b.object_id, target_label))
                        continue
                    ...
                    for distractor in target_objects:
                        if distractor.object_id == target.object_id:
                            continue
                        if (
                            target.distance_from_other_object(distractor, optimized=True)
                            < safe_distance
                        ):
                            target_away_from_others = False
                            bad_combinations.append(
                                (anc_a.object_id, anc_b.object_id, target_label)
                            )
                            break
                    ...
                    if is_between:
                        ref = Reference(
                            target, anc_a, reference_type="between", second_anchor=anc_b
                        )
                        all_refs.append(ref)
    ...
    def is_between_candidate(
        ...
        target_anchor_intersect_ratio_thresh: float,
    ) -> (bool, bool):
        """
        ...
        if (
            occ_ratio < occ_thresh
        ):  # The object is not in the convex-hull enough to be considered between
