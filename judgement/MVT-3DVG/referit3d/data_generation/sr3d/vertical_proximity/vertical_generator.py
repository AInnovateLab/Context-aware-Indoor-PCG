    ...
    def generate_for_single_scan(self, scan):
        ...
        for anchor in all_anchors:
            ...
            for target in all_targets:
                ...
                target_can_support = (
                    self.instance_can_support[target.instance_label].lower() == "true"
                )
                anchor_can_support = (
                    self.instance_can_support[anchor.instance_label].lower() == "true"
                )

                # Is the target/anchor cannot be supported by anything other than the walls/floor?
                target_cannot_be_supported = (
                    self.instance_cannot_be_supported[target.instance_label].lower() == "true"
                )
                anchor_cannot_be_supported = (
                    self.instance_cannot_be_supported[anchor.instance_label].lower() == "true"
                )
                ...
                if (
                    anchor_can_support
                    and i_target_ratio > min_to_be_supported_area_ratio
                    and abs(target_bottom_anchor_top_dist) <= max_to_be_touching_distance
                    and not target_cannot_be_supported
                    and target_anchor_area_ratio < max_to_be_supporting_area_ratio
                ):  # target is not quite larger in area
                    # than the anchor
                ...
                if (
                    target_can_support
                    and i_anchor_ratio > min_to_be_supported_area_ratio
                    and abs(target_top_anchor_bottom_dist) <= max_to_be_touching_distance
                    and not anchor_cannot_be_supported
                    and anchor_target_area_ratio < max_to_be_supporting_area_ratio
                ):
                    target_supporting_anchor = True

                target_above_anchor = (
                    target_bottom_anchor_top_dist > min_above_below_distance
                    and max(i_anchor_ratio, i_target_ratio) > min_to_be_above_below_area_ratio
                )  # above
                target_below_anchor = (
                    target_top_anchor_bottom_dist > min_above_below_distance
                    and max(i_anchor_ratio, i_target_ratio) > min_to_be_above_below_area_ratio
                )  # below
        ...
        for r in all_refs:
            ...
            for i in range(len(distractors)):
                for j in range(i + 1, len(distractors)):
                    i_bbox, j_bbox = distractors[i].get_bbox(axis_aligned=True), distractors[
                        j
                    ].get_bbox(axis_aligned=True)
