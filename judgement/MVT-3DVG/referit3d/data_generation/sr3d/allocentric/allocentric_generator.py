...
class AllocentricGenerator(ReferenceGenerator):
            ...
            for target_instance_label in targets.keys():
                ...
                for target in candidate_targets.values():
                    ...
                    for point in target_points:
                        section_id = self.which_section_point_in(
                            anchor.get_bbox(), anchor_sections, point
                        ).value
                ...
                for sec, in_sec_targets in oriented_sections.items():
                    ...
                    if (
                        len(occupying_targets) == 1
                        and n_non_occpying_targets == len(in_sec_targets) - 1
                    ):
                        candidate_refs.append(
                            Reference(occupying_targets[0], anchor, OrientedSections(sec).name)
                        )
    ...
    def get_anchor_sections(extrema, a, dl, df, d2):
        ...
        section_names = [
            OrientedSections.front,
            OrientedSections.back,
            OrientedSections.right,
            OrientedSections.left,
        ]
        ret = {}
