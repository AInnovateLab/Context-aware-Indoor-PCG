...
def pre_process_text(
    text, manual_sentence_speller, manual_token_speller, tokenizer, golden_vocabulary, token_speller
):
    missed_words = set()
    corrected = defaultdict(list)

    clean_text = text.apply(
        lambda x: manual_sentence_spelling(x, manual_sentence_speller)
    )  # sentence-to-sentence map
    ...
    def spell_if_not_in_golden(token_list):
        ...
        for token in token_list:
            ...
            if token not in golden_vocabulary:
                new_tokens.append(
                    token_spell_check(
                        token, token_speller, corrected, missed_words, utterance=token_list
                    )
                )
            else:
                new_tokens.append(token)
    ...
    spelled_tokens = tokens.apply(
        lambda x: manual_tokenized_sentence_spelling(x, spelling_dictionary=manual_token_speller)
    )
